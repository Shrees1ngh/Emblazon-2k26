...
export default function DomeGallery({
    ...
    autoRotateSpeed = 4,
    pointerFollow = true,
    pointerFollowStrength = 0.6,
    pointerFollowEase = 0.08
}) {
    ...
    useEffect(() => {
        ...
        const tick = now => {
            ...
            if (
                ...
                const baseY = rotationRef.current.y + (autoRotate ? (autoRotateSpeed * dt) / 1000 : 0)
                const targetY = wrapAngleSigned(baseY + pointerX * 12 * pointerFollowStrength)
                const deltaY = wrapAngleSigned(targetY - rotationRef.current.y)
                const nextY = wrapAngleSigned(rotationRef.current.y + deltaY * pointerFollowEase)
                const targetX = clamp(
                    -pointerY * maxVerticalRotationDeg * pointerFollowStrength,
                    -maxVerticalRotationDeg,
                    maxVerticalRotationDeg
                )
                const nextX = rotationRef.current.x + (targetX - rotationRef.current.x) * pointerFollowEase
